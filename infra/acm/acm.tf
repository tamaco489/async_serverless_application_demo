...
# API Gatewayのカスタムドメインの定義（ユーザー情報管理サービスAPI向け）
resource "aws_apigatewayv2_domain_name" "coral_api_http_v3" {
  domain_name = "apiv3.${data.terraform_remote_state.route53.outputs.host_zone.name}"
...
# API V3向けのRoute53 Aレコードを設定（ユーザー情報管理サービスAPI向け）
resource "aws_route53_record" "coral_api_http_v3" {
  zone_id = data.terraform_remote_state.route53.outputs.host_zone.id
  name    = "apiv3.${data.terraform_remote_state.route53.outputs.host_zone.name}"
  type    = "A"

  alias {
    name                   = aws_apigatewayv2_domain_name.coral_api_http_v3.domain_name_configuration.0.target_domain_name
    zone_id                = aws_apigatewayv2_domain_name.coral_api_http_v3.domain_name_configuration.0.hosted_zone_id
    evaluate_target_health = false
  }
}
